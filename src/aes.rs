// Minimal AES-128 block encryptor for emulating the PHY6252 vendor-ROM crypto ABI.
// This is deliberately dependency-free: the emulator needs one deterministic 16-byte primitive,
// not a general crypto API.

const SBOX: [u8; 256] = [
    ,
];

const RCON: [u8; 10] = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36];

pub fn aes128_encrypt_block(key: [u8; 16], plaintext: [u8; 16]) -> [u8; 16] {
    let round_keys = expand_key(key);
    let mut state = plaintext;
    add_round_key(&mut state, &round_keys[0..16]);

    for round in 1..10 {
        sub_bytes(&mut state);
        shift_rows(&mut state);
        mix_columns(&mut state);
        add_round_key(&mut state, &round_keys[round * 16..(round + 1) * 16]);
    }

    sub_bytes(&mut state);
    shift_rows(&mut state);
    add_round_key(&mut state, &round_keys[160..176]);
    state
}

fn expand_key(key: [u8; 16]) -> [u8; 176] {
    let mut out = [0u8; 176];
    out[..16].copy_from_slice(&key);
    let mut generated = 16;
    let mut rcon = 0;
    let mut temp = [0u8; 4];

    while generated < out.len() {
        temp.copy_from_slice(&out[generated - 4..generated]);
        if generated % 16 == 0 {
            temp.rotate_left(1);
            for b in &mut temp {
                *b = SBOX[*b as usize];
            }
            temp[0] ^= RCON[rcon];
            rcon += 1;
        }
        for b in temp {
            out[generated] = out[generated - 16] ^ b;
            generated += 1;
        }
    }
    out
}

fn add_round_key(state: &mut [u8; 16], key: &[u8]) {
    for (dst, src) in state.iter_mut().zip(key.iter()) {
        *dst ^= *src;
    }
}

fn sub_bytes(state: &mut [u8; 16]) {
    for b in state {
        *b = SBOX[*b as usize];
    }
}

fn shift_rows(s: &mut [u8; 16]) {
    let old = *s;
    s[1] = old[5]; s[5] = old[9]; s[9] = old[13]; s[13] = old[1];
    s[2] = old[10]; s[6] = old[14]; s[10] = old[2]; s[14] = old[6];
    s[3] = old[15]; s[7] = old[3]; s[11] = old[7]; s[15] = old[11];
}

fn xtime(x: u8) -> u8 {
    (x << 1) ^ if x & 0x80 != 0 { 0x1b } else { 0 }
}

fn mix_columns(s: &mut [u8; 16]) {
    for col in s.chunks_exact_mut(4) {
        let a0 = col[0];
        let a1 = col[1];
        let a2 = col[2];
        let a3 = col[3];
        let t = a0 ^ a1 ^ a2 ^ a3;
        col[0] ^= t ^ xtime(a0 ^ a1);
        col[1] ^= t ^ xtime(a1 ^ a2);
        col[2] ^= t ^ xtime(a2 ^ a3);
        col[3] ^= t ^ xtime(a3 ^ a0);
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn fips_197_aes128_vector() {
        let key = [
            0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
            0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,
        ];
        let plaintext = [
            0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,
            0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff,
        ];
        assert_eq!(
            aes128_encrypt_block(key, plaintext),
            [
                0x69,0xc4,0xe0,0xd8,0x6a,0x7b,0x04,0x30,
                0xd8,0xcd,0xb7,0x80,0x70,0xb4,0xc5,0x5a,
            ]
        );
    }
}
