...
impl HostOsal {
    ...
    fn start(&mut self, cpu: &mut Processor) -> bool {
        ...
        if !self.started {
            ...
            self.started = true; self.once(ROM_START, || eprintln!("OSAL host cooperative scheduler started"));
        }
        self.dispatch(cpu)
    }
    ...
    fn dispatch(&mut self, cpu: &mut Processor) -> bool {
        ...
        for task in 0..self.count {
            ...
            if bits == 0 { continue; }
            let handler = match cpu.read32(tasks.wrapping_add(u32::from(task) * 4)) { Ok(v) if v & 1 == 1 => v, _ => return false };
            if cpu.write16(event_addr, 0).is_err() { return false; }
            self.running = Some(task); cpu.set_r(Reg::R0, u32::from(task)); cpu.set_r(Reg::R1, u32::from(bits));
            cpu.set_r(Reg::LR, ROM_START | 1); cpu.set_pc(handler & !1); return true;
        }
        cpu.set_r(Reg::LR, IDLE_BX_LR_ROM | 1); cpu.set_pc(IDLE_BX_LR_ROM); true
    }

    fn event_addr(&self, task: u8) -> Option<u32> {
        if task >= self.count { return None; } self.events.map(|p| p + u32::from(task) * 2)
    }
    ...
    fn set_event_call(&mut self, cpu: &mut Processor) -> bool {
        if !self.resolve(cpu) { return false; }
        let task = cpu.get_r(Reg::R0) as u8; let event = cpu.get_r(Reg::R1) as u16;
    ...
    fn timer_call(&mut self, cpu: &mut Processor, now: u32, reload: bool) -> bool {
        if !self.resolve(cpu) { return false; }
        let task = cpu.get_r(Reg::R0) as u8; let event = cpu.get_r(Reg::R1) as u16; let ms = cpu.get_r(Reg::R2);
        if task >= self.count || event == 0 { return false; }
        self.timers.retain(|t| !(t.task == task && t.event == event));
        if ms == 0 { if !self.post(cpu, task, event) { return false; } }
        else { self.timers.push(Timer { task, event, deadline: now.wrapping_add(ms), reload: if reload { ms } else { 0 } }); }
    ...
    fn stop_timer(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8; let event = cpu.get_r(Reg::R1) as u16;
        self.timers.retain(|t| !(t.task == task && t.event == event)); self.once(ROM_STOP_TIMER, || eprintln!("OSAL host stop_timer"));
        cpu.set_r(Reg::R0, 0); ret(cpu); true
    }
    fn expire(&mut self, cpu: &mut Processor, now: u32) {
        let mut due = Vec::new(); for (i, t) in self.timers.iter().enumerate() { if reached(now, t.deadline) { due.push(i); } }
        for i in due.into_iter().rev() {
            let mut t = self.timers.remove(i); let _ = self.post(cpu, t.task, t.event);
            if t.reload != 0 { loop { t.deadline = t.deadline.wrapping_add(t.reload); if !reached(now, t.deadline) { break; } } self.timers.push(t); }
        }
    }
    ...
    fn heap(&mut self, cpu: &mut Processor) -> bool {
        ...
        self.heap_next = Some(align4(base)); self.heap_end = end; eprintln!("OSAL host heap base={base:#010x} size={size:#x}"); true
    }
    fn alloc_block(&mut self, cpu: &mut Processor, requested: u32) -> u32 {
        if requested == 0 || !self.heap(cpu) { return 0; } let size = align4(requested);
        if let Some((index, &(ptr, available))) = self.free.iter().enumerate().find(|(_, block)| block.1 >= size) {
            self.free.swap_remove(index); if available > size { self.free.push((ptr + size, available - size)); }
            self.allocs.insert(ptr, size); return ptr;
        }
        let start = self.heap_next.unwrap(); let Some(end) = start.checked_add(size) else { return 0; };
        if end > self.heap_end { return 0; } self.heap_next = Some(end); self.allocs.insert(start, size); start
    }
    fn free_block(&mut self, ptr: u32) -> bool {
        if ptr == 0 { return true; } let Some(size) = self.allocs.remove(&ptr) else { return false; }; self.free.push((ptr, size)); true
    }
    ...
    fn free_call(&mut self, cpu: &mut Processor) -> bool {
        let ptr = cpu.get_r(Reg::R0); if !self.free_block(ptr) { eprintln!("OSAL strict free unknown ptr={ptr:#010x}"); return false; }
        self.once(ROM_FREE, || eprintln!("OSAL host mem_free")); ret(cpu); true
    }

    fn memcpy_bytes(cpu: &mut Processor, dst: u32, src: u32, len: u32, reverse: bool) -> bool {
        let mut bytes = Vec::with_capacity(len as usize);
        for i in 0..len { let at = if reverse { len - 1 - i } else { i }; let Ok(v) = cpu.read8(src.wrapping_add(at)) else { return false; }; bytes.push(v); }
        for (i, byte) in bytes.into_iter().enumerate() { let at = if reverse { len - 1 - i as u32 } else { i as u32 }; if cpu.write8(dst.wrapping_add(at), byte).is_err() { return false; } }
        true
    }
    fn memcpy_call(&mut self, cpu: &mut Processor) -> bool { let dst=cpu.get_r(Reg::R0); let src=cpu.get_r(Reg::R1); let len=cpu.get_r(Reg::R2); if !Self::memcpy_bytes(cpu,dst,src,len,false){return false;} self.once(ROM_MEMCPY,||eprintln!("OSAL host memcpy")); cpu.set_r(Reg::R0,dst); ret(cpu); true }
    fn revmemcpy_call(&mut self, cpu: &mut Processor) -> bool { let dst=cpu.get_r(Reg::R0); let src=cpu.get_r(Reg::R1); let len=cpu.get_r(Reg::R2); if !Self::memcpy_bytes(cpu,dst,src,len,true){return false;} self.once(ROM_REVMEMCPY,||eprintln!("OSAL host revmemcpy")); cpu.set_r(Reg::R0,dst); ret(cpu); true }
    fn memdup_call(&mut self, cpu: &mut Processor) -> bool { let src=cpu.get_r(Reg::R0); let len=cpu.get_r(Reg::R1); let dst=self.alloc_block(cpu,len); if dst!=0&&!Self::memcpy_bytes(cpu,dst,src,len,false){return false;} self.once(ROM_MEMDUP,||eprintln!("OSAL host memdup")); cpu.set_r(Reg::R0,dst); ret(cpu); true }
    ...
    fn msg_alloc(&mut self, cpu: &mut Processor) -> bool {
        let len=cpu.get_r(Reg::R0); let hdr=self.alloc_block(cpu,len.saturating_add(MSG_HDR)); let payload=if hdr==0{0}else{hdr+MSG_HDR};
        if hdr!=0 && (cpu.write32(hdr,0).is_err()||cpu.write16(hdr+MSG_LEN_OFF,len as u16).is_err()||cpu.write8(hdr+MSG_DEST_OFF,INVALID_TASK).is_err()){return false;}
        self.once(ROM_MSG_ALLOC,||eprintln!("OSAL host message allocation")); cpu.set_r(Reg::R0,payload); ret(cpu); true
    }
    ...
    fn msg_send(&mut self, cpu: &mut Processor) -> bool {
        if !self.resolve(cpu){return false;} let task=cpu.get_r(Reg::R0) as u8; let payload=cpu.get_r(Reg::R1);
        if task>=self.count||payload<MSG_HDR{cpu.set_r(Reg::R0,1);ret(cpu);return true;} let hdr=payload-MSG_HDR;
        ...
        self.messages.push_back(payload); if !self.post(cpu,task,SYS_EVENT_MSG){return false;} self.once(ROM_MSG_SEND,||eprintln!("OSAL host message queue send + SYS_EVENT_MSG")); cpu.set_r(Reg::R0,0); ret(cpu); true
    }
    fn msg_receive(&mut self, cpu: &mut Processor) -> bool {
        let task=cpu.get_r(Reg::R0) as u8; let pos=self.messages.iter().position(|payload|cpu.read8(*payload-MSG_HDR+MSG_DEST_OFF).ok()==Some(task));
        let payload=pos.and_then(|i|self.messages.remove(i)).unwrap_or(0); if payload!=0{let _=cpu.write32(payload-MSG_HDR,0);} self.once(ROM_MSG_RECEIVE,||eprintln!("OSAL host message queue receive")); cpu.set_r(Reg::R0,payload); ret(cpu); true
    }

    fn flash_write(&mut self, cpu: &mut Processor, dma: bool) -> bool {
        let addr=cpu.get_r(Reg::R0); let src=cpu.get_r(Reg::R1); let len=cpu.get_r(Reg::R2);
        if (addr as usize).saturating_add(len as usize)>XIP_SIZE { cpu.set_r(Reg::R0,1); ret(cpu); return true; }
        if cpu.write32(HOST_FLASH_ADDR,addr).is_err(){return false;}
        for i in 0..len { let byte=match cpu.read8(src.wrapping_add(i)){Ok(v)=>v,Err(_)=>return false}; if cpu.write32(HOST_FLASH_PROGRAM,u32::from(byte)).is_err(){return false;} }
        let entry=if dma{ROM_SPIF_WRITE_DMA}else{ROM_SPIF_WRITE}; self.once(entry,||eprintln!("FLASH host {} program 1->0",if dma{"DMA"}else{"PIO"})); cpu.set_r(Reg::R0,0); ret(cpu); true
    }
    ...
    fn flash_erase(&mut self, cpu:&mut Processor, bytes:u32)->bool {
        let addr=cpu.get_r(Reg::R0); if (addr as usize)>=XIP_SIZE {cpu.set_r(Reg::R0,1);ret(cpu);return true;}
        let start=addr&!(FLASH_SECTOR-1); let end=start.saturating_add(bytes).min(XIP_SIZE as u32); let mut at=start;
        while at<end {if !Self::erase_sector_at(cpu,at){return false;} at=at.saturating_add(FLASH_SECTOR);}
        let entry=if bytes==FLASH_SECTOR{ROM_SPIF_ERASE_SECTOR}else{ROM_SPIF_ERASE_BLOCK64}; self.once(entry,||eprintln!("FLASH host erase bytes={bytes:#x}")); cpu.set_r(Reg::R0,0); ret(cpu); true
    }
...
mod tests {
    ...
    #[test] fn timer_key_is_task_and_event(){let a=Timer{task:1,event:2,deadline:3,reload:0};let b=Timer{task:2,event:2,deadline:3,reload:0};assert_ne!(a.task,b.task);assert_eq!(a.event,b.event);}
    #[test] fn free_block_can_be_reused(){let mut h=HostOsal{heap_next:Some(0x1000),heap_end:0x1100,..HostOsal::default()};h.allocs.insert(0x1000,16);h.free_block(0x1000);assert_eq!(h.free,vec![(0x1000,16)]);}
}
