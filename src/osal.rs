...
const ROM_SPIF_CMD: u32 = 0x0001_6D48;
...
const ROM_SPIF_RDDATA: u32 = 0x0001_713C;
const ROM_SPIF_WRDATA: u32 = 0x0001_736C;
...
const FCMD_WREN: u8 = 0x06;
const FCMD_WRDIS: u8 = 0x04;
const FCMD_VSRWREN: u8 = 0x50;
...
pub struct HostOsal {
    ...
    ll_task: Option<u8>,
    hci_task: Option<u8>,
    cb_timer_task: Option<u8>,
    hci_ext_task: Option<u8>,
    gap_task: Option<u8>,
    l2cap_task: Option<u8>,
    smp_task: Option<u8>,
    test_app_task: Option<u8>,
    spif_read: [u8; 8],
    spif_read_len: u8,
    spif_write: [u8; 8],
    spif_write_len: u8,
...
impl HostOsal {
    ...
    pub fn handle(&mut self, cpu: &mut Processor) -> bool {
        if !flash_state::ensure_loaded(cpu) {
            return false;
        }
        let now = simulated_ms(cpu);
        self.expire(cpu, now);
        if ble_rom::handle(cpu, &mut self.rng) {
            return true;
        }
        let pc = cpu.get_pc();
        ...
        match pc {
            ROM_UIDIV => self.uidiv(cpu),
            ROM_IDIV => self.idiv(cpu),
            ROM_HCI_EXT_TASK_REGISTER => self.hci_ext_task_register(cpu),
            ROM_HCI_GAP_TASK_REGISTER => self.hci_gap_task_register(cpu),
            ROM_HCI_INIT => self.hci_init(cpu),
            ROM_HCI_L2CAP_TASK_REGISTER => self.hci_l2cap_task_register(cpu),
            ROM_HCI_SMP_TASK_REGISTER => self.hci_smp_task_register(cpu),
            ROM_HCI_TEST_APP_TASK_REGISTER => self.hci_test_app_task_register(cpu),
            ROM_LL_INIT => self.ll_init(cpu),
            ROM_CB_TIMER_INIT => self.cb_timer_init(cpu),
            ROM_CLOCK => self.clock(cpu, now),
            ROM_BUFFER_UINT24 => self.buffer_uint(cpu, 3),
            ROM_BUFFER_UINT32 => self.buffer_uint(cpu, 4),
            ROM_BUILD_UINT16 => self.build_uint16(cpu),
            ROM_BUILD_UINT32 => self.build_uint32(cpu),
            ROM_CLEAR_EVENT => self.clear_event_call(cpu),
            ROM_GET_TIMEOUT => self.get_timeout(cpu, now),
            ROM_NEXT_TIMEOUT => self.next_timeout(cpu, now),
            ROM_ISBUFSET => self.isbufset(cpu),
            ROM_RAND => self.rand_call(cpu),
            ...
            ROM_SPIF_CMD => self.spif_cmd(cpu),
            ROM_SPIF_RDDATA => self.spif_rddata(cpu),
            ROM_SPIF_WRDATA => self.spif_wrdata(cpu),
            ROM_SPIF_WRITE => self.flash_write(cpu, false),
            ROM_SPIF_WRITE_DMA => self.flash_write(cpu, true),
            ROM_SPIF_ERASE_SECTOR => self.flash_erase(cpu, FLASH_SECTOR),
            ROM_SPIF_ERASE_BLOCK64 => self.flash_erase(cpu, FLASH_BLOCK64),
            ROM_SPIF_ERASE_ALL => self.flash_erase_all(cpu),
            _ => false,
        }
    }
    ...
    fn ll_init(&mut self, cpu: &mut Processor) -> bool {
        ...
        self.once(ROM_LL_INIT, || {
            eprintln!("BLE host controller initialized by guest LL task={task}")
        });
        ret(cpu);
        true
    }
    fn hci_init(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        self.hci_task = Some(task);
        self.once(ROM_HCI_INIT, || {
            eprintln!("BLE host HCI initialized by guest task={task}")
        });
        ret(cpu);
        true
    }
    fn cb_timer_init(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        self.cb_timer_task = Some(task);
        self.once(ROM_CB_TIMER_INIT, || {
            eprintln!("OSAL callback timer task initialized task={task}")
        });
        ret(cpu);
        true
    }
    fn hci_ext_task_register(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        self.hci_ext_task = Some(task);
        self.once(ROM_HCI_EXT_TASK_REGISTER, || {
            eprintln!("BLE HCI route EXT task={task}")
        });
        ret(cpu);
        true
    }
    fn hci_gap_task_register(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        self.gap_task = Some(task);
        self.once(ROM_HCI_GAP_TASK_REGISTER, || {
            eprintln!("BLE HCI route GAP task={task}")
        });
        ret(cpu);
        true
    }
    fn hci_l2cap_task_register(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        self.l2cap_task = Some(task);
        self.once(ROM_HCI_L2CAP_TASK_REGISTER, || {
            eprintln!("BLE HCI route L2CAP task={task}")
        });
        ret(cpu);
        true
    }
    fn hci_smp_task_register(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        self.smp_task = Some(task);
        self.once(ROM_HCI_SMP_TASK_REGISTER, || {
            eprintln!("BLE HCI route SMP task={task}")
        });
        ret(cpu);
        true
    }
    fn hci_test_app_task_register(&mut self, cpu: &mut Processor) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        self.test_app_task = Some(task);
        self.once(ROM_HCI_TEST_APP_TASK_REGISTER, || {
            eprintln!("BLE HCI route test-app task={task}")
        });
        ret(cpu);
        true
    }

    fn buffer_uint(&mut self, cpu: &mut Processor, len: u32) -> bool {
        let ptr = cpu.get_r(Reg::R0);
        let value = cpu.get_r(Reg::R1);
        for i in 0..len {
            if cpu
                .write8(ptr + i, ((value >> (8 * i)) & 0xff) as u8)
                .is_err()
            {
                return false;
            }
        ...
        self.once(entry, || {
            eprintln!("OSAL host buffer_uint{} little-endian", len * 8)
        });
        cpu.set_r(Reg::R0, ptr + len);
        ret(cpu);
        true
    }
    fn build_uint16(&mut self, cpu: &mut Processor) -> bool {
        let ptr = cpu.get_r(Reg::R0);
        let lo = match cpu.read8(ptr) {
            Ok(v) => v as u32,
            Err(_) => return false,
        };
        let hi = match cpu.read8(ptr + 1) {
            Ok(v) => v as u32,
            Err(_) => return false,
        };
        self.once(ROM_BUILD_UINT16, || {
            eprintln!("OSAL host build_uint16 little-endian")
        });
        cpu.set_r(Reg::R0, lo | (hi << 8));
        ret(cpu);
        true
    }
    fn build_uint32(&mut self, cpu: &mut Processor) -> bool {
        let ptr = cpu.get_r(Reg::R0);
        let len = (cpu.get_r(Reg::R1) as u8).min(4);
        let mut value = 0u32;
        for i in 0..len {
            let b = match cpu.read8(ptr + u32::from(i)) {
                Ok(v) => v as u32,
                Err(_) => return false,
            };
            value |= b << (8 * u32::from(i));
        }
        self.once(ROM_BUILD_UINT32, || {
            eprintln!("OSAL host build_uint32 little-endian")
        });
        cpu.set_r(Reg::R0, value);
        ret(cpu);
        true
    }
    fn isbufset(&mut self, cpu: &mut Processor) -> bool {
        let ptr = cpu.get_r(Reg::R0);
        let value = cpu.get_r(Reg::R1) as u8;
        let len = cpu.get_r(Reg::R2) as u8;
        let mut yes = true;
        for i in 0..u32::from(len) {
            match cpu.read8(ptr + i) {
                Ok(v) if v == value => {}
                Ok(_) => {
                    yes = false;
                    break;
        ...
        self.once(ROM_ISBUFSET, || eprintln!("OSAL host isbufset"));
        cpu.set_r(Reg::R0, u32::from(yes));
        ret(cpu);
        true
    }
    fn rand_call(&mut self, cpu: &mut Processor) -> bool {
        let mut x = if self.rng == 0 { 0x6252_A5A5 } else { self.rng };
        x ^= x << 13;
        x ^= x >> 17;
        x ^= x << 5;
        self.rng = x;
        self.once(ROM_RAND, || eprintln!("OSAL host deterministic PRNG"));
        cpu.set_r(Reg::R0, x & 0xffff);
        ret(cpu);
        true
    }

    fn spif_cmd(&mut self, cpu: &mut Processor) -> bool {
        let op = cpu.get_r(Reg::R0) as u8;
        let rdlen = cpu.get_r(Reg::R2) as u8;
        self.spif_read = [0; 8];
        self.spif_read_len = 0;
        match op {
            ...
            _ if rdlen == 0 => {}
            _ => {
                eprintln!("FLASH strict unsupported raw spif_cmd op={op:#04x} rdlen={rdlen}");
                return false;
            }
        }
    ...
    fn spif_rddata(&mut self, cpu: &mut Processor) -> bool {
        let dst = cpu.get_r(Reg::R0);
        let len = cpu.get_r(Reg::R1) as u8;
        if len > self.spif_read_len || len > 8 {
            eprintln!(
                "FLASH strict spif_rddata len={len} prepared={}",
                self.spif_read_len
            );
            return false;
        }
        for i in 0..len {
            if cpu
                .write8(dst + u32::from(i), self.spif_read[i as usize])
                .is_err()
            {
                return false;
            }
        }
        self.once(ROM_SPIF_RDDATA, || eprintln!("FLASH host raw spif_rddata"));
        ret(cpu);
        true
    }
    fn spif_wrdata(&mut self, cpu: &mut Processor) -> bool {
        let src = cpu.get_r(Reg::R0);
        let len = (cpu.get_r(Reg::R1) as u8).min(8);
        self.spif_write = [0; 8];
        for i in 0..len {
            self.spif_write[i as usize] = match cpu.read8(src + u32::from(i)) {
                Ok(v) => v,
                Err(_) => return false,
            };
        }
        self.spif_write_len = len;
        self.once(ROM_SPIF_WRDATA, || {
            eprintln!("FLASH host raw spif_wrdata staging")
        });
        ret(cpu);
        true
    }
    ...
    fn init(&mut self, cpu: &mut Processor) -> bool {
        ...
        self.once(ROM_INIT, || {
            eprintln!("OSAL host init task_init={entry:#010x}")
        });
        cpu.set_pc(entry & !1);
        true
    }
    fn start(&mut self, cpu: &mut Processor) -> bool {
        ...
        if !self.started {
            ...
            self.started = true;
            self.once(ROM_START, || {
                eprintln!("OSAL host cooperative scheduler started")
            });
        }
        self.dispatch(cpu)
    }
    ...
    fn set_event_call(&mut self, cpu: &mut Processor) -> bool {
        ...
        self.once(ROM_SET_EVENT, || {
            eprintln!("OSAL host set_event -> guest event bitmap")
        });
        cpu.set_r(Reg::R0, 0);
        ret(cpu);
        true
    ...
    fn clear_event_call(&mut self, cpu: &mut Processor) -> bool {
        ...
        if cpu.write16(addr, current & !event).is_err() {
            return false;
        }
        self.once(ROM_CLEAR_EVENT, || {
            eprintln!("OSAL host clear_event -> guest event bitmap")
        });
        cpu.set_r(Reg::R0, 0);
        ret(cpu);
        true
    ...
    fn remaining(now: u32, deadline: u32) -> u32 {
    ...
    fn get_timeout(&mut self, cpu: &mut Processor, now: u32) -> bool {
        let task = cpu.get_r(Reg::R0) as u8;
        let event = cpu.get_r(Reg::R1) as u16;
        let value = self
            .timers
            .iter()
            .find(|t| t.task == task && t.event == event)
            .map(|t| Self::remaining(now, t.deadline))
            .unwrap_or(0);
        self.once(ROM_GET_TIMEOUT, || eprintln!("OSAL host get_timeout"));
        cpu.set_r(Reg::R0, value);
        ret(cpu);
        true
    }
    fn next_timeout(&mut self, cpu: &mut Processor, now: u32) -> bool {
        let value = self
            .timers
            .iter()
            .map(|t| Self::remaining(now, t.deadline))
            .min()
            .unwrap_or(0);
        self.once(ROM_NEXT_TIMEOUT, || eprintln!("OSAL host next_timeout"));
        cpu.set_r(Reg::R0, value);
        ret(cpu);
        true
    }
    fn timer_num_active(&mut self, cpu: &mut Processor) -> bool {
        cpu.set_r(Reg::R0, self.timers.len().min(255) as u32);
        ret(cpu);
        true
    }
    fn timer_call(&mut self, cpu: &mut Processor, now: u32, reload: bool) -> bool {
        ...
        self.timers
            .retain(|t| !(t.task == task && t.event == event));
        if ms == 0 {
            if !self.post(cpu, task, event) {
                return false;
            }
    ...
    fn alloc_block(&mut self, cpu: &mut Processor, requested: u32) -> u32 {
        if requested == 0 || !self.heap(cpu) {
            return 0;
        }
        let size = align4(requested);
        if let Some((index, &(ptr, available))) = self
            .free
            .iter()
            .enumerate()
            .find(|(_, block)| block.1 >= size)
        {
            self.free.swap_remove(index);
            if available > size {
                self.free.push((ptr + size, available - size));
            }
    ...
    fn free_block(&mut self, ptr: u32) -> bool {
        ...
        let Some(size) = self.allocs.remove(&ptr) else {
            return false;
        };
    ...
    fn free_call(&mut self, cpu: &mut Processor) -> bool {
        ...
        if !self.free_block(ptr) {
            eprintln!("OSAL strict free unknown ptr={ptr:#010x}");
            return false;
        }
    ...
    fn memcpy_bytes(cpu: &mut Processor, dst: u32, src: u32, len: u32) -> bool {
        let mut bytes = Vec::with_capacity(len as usize);
        for i in 0..len {
            let Ok(v) = cpu.read8(src.wrapping_add(i)) else {
                return false;
            };
            bytes.push(v);
        }
    ...
    fn memcpy_call(&mut self, cpu: &mut Processor) -> bool {
        ...
        if !Self::memcpy_bytes(cpu, dst, src, len) {
            return false;
        }
    ...
    fn revmemcpy_call(&mut self, cpu: &mut Processor) -> bool {
        ...
        let len = cpu.get_r(Reg::R2);
        for i in 0..len {
            let byte = match cpu.read8(src.wrapping_add(len - 1 - i)) {
                Ok(v) => v,
                Err(_) => return false,
            };
            if cpu.write8(dst.wrapping_add(i), byte).is_err() {
                return false;
            }
        }
    ...
    fn memdup_call(&mut self, cpu: &mut Processor) -> bool {
        let src = cpu.get_r(Reg::R0);
        let len = cpu.get_r(Reg::R1);
        let dst = self.alloc_block(cpu, len);
        if dst != 0 && !Self::memcpy_bytes(cpu, dst, src, len) {
            return false;
        }
    ...
    fn msg_alloc(&mut self, cpu: &mut Processor) -> bool {
        ...
        if hdr != 0
            && (cpu.write32(hdr, 0).is_err()
                || cpu.write16(hdr + MSG_LEN_OFF, len as u16).is_err()
                || cpu.write8(hdr + MSG_DEST_OFF, INVALID_TASK).is_err())
        {
            return false;
        }
    ...
    fn msg_dealloc(&mut self, cpu: &mut Processor) -> bool {
        ...
        self.once(ROM_MSG_DEALLOC, || {
            eprintln!("OSAL host message deallocation")
        });
        cpu.set_r(Reg::R0, if ok { 0 } else { 1 });
        ret(cpu);
        true
    }
    fn msg_send(&mut self, cpu: &mut Processor) -> bool {
        ...
        let task = cpu.get_r(Reg::R0) as u8;
        let payload = cpu.get_r(Reg::R1);
        if task >= self.count || payload < MSG_HDR {
            cpu.set_r(Reg::R0, 1);
            ret(cpu);
            return true;
        }
        let hdr = payload - MSG_HDR;
        if !self.allocs.contains_key(&hdr)
            || cpu.write8(hdr + MSG_DEST_OFF, task).is_err()
            || cpu.write32(hdr, 0).is_err()
        {
            cpu.set_r(Reg::R0, 1);
            ret(cpu);
        ...
        if !self.post(cpu, task, SYS_EVENT_MSG) {
            return false;
        }
        self.once(ROM_MSG_SEND, || {
            eprintln!("OSAL host message queue send + SYS_EVENT_MSG")
        });
        cpu.set_r(Reg::R0, 0);
        ret(cpu);
        true
    ...
    fn flash_write(&mut self, cpu: &mut Processor, dma: bool) -> bool {
        ...
        for i in 0..len {
            let byte = match cpu.read8(src.wrapping_add(i)) {
                Ok(v) => v,
                Err(_) => return false,
            };
    ...
    fn erase_sector_at(cpu: &mut Processor, addr: u32) -> bool {
    ...
    fn flash_erase(&mut self, cpu: &mut Processor, bytes: u32) -> bool {
        ...
        let end = start.saturating_add(bytes).min(XIP_SIZE as u32);
        let mut at = start;
        while at < end {
            if !Self::erase_sector_at(cpu, at) {
                return false;
            }
            at = at.saturating_add(FLASH_SECTOR);
        }
        if !flash_state::persist(cpu) {
            return false;
        }
        let entry = if bytes == FLASH_SECTOR {
            ROM_SPIF_ERASE_SECTOR
        } else {
            ROM_SPIF_ERASE_BLOCK64
        };
        self.once(entry, || eprintln!("FLASH host erase bytes={bytes:#x}"));
        cpu.set_r(Reg::R0, 0);
        ret(cpu);
        true
    }
    fn flash_erase_all(&mut self, cpu: &mut Processor) -> bool {
        let mut at = 0u32;
        while (at as usize) < XIP_SIZE {
            if !Self::erase_sector_at(cpu, at) {
                return false;
            }
            at += FLASH_SECTOR;
        ...
        self.once(ROM_SPIF_ERASE_ALL, || eprintln!("FLASH host chip erase"));
        cpu.set_r(Reg::R0, 0);
        ret(cpu);
        true
    }
...
mod tests {
    ...
    fn message_header_matches_arm_osal_layout() {
        assert_eq!(MSG_HDR, 8);
        assert_eq!(MSG_LEN_OFF, 4);
        assert_eq!(MSG_DEST_OFF, 6);
    }
    #[test]
    ...
    #[test]
    fn free_block_can_be_reused() {
        let mut h = HostOsal {
            heap_next: Some(0x1000),
            heap_end: 0x1100,
            ..HostOsal::default()
        };
