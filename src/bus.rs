...
impl Phy6252Bus {
    ...
    fn flash_program_byte(&mut self, value: u8) {
        let addr = self.flash_addr.get();
        if let Some(off) = self.flash_offset(addr) {
            self.xip[off] &= value;
            self.flash_addr.set(addr.wrapping_add(1));
        }
    }
...
mod tests {
    ...
    #[test]
    fn nor_program_only_clears_bits_and_direct_xip_store_is_ignored() {
        let mut bus = Phy6252Bus::new(vec![0; SRAM_SIZE], vec![0xff; XIP_SIZE]);
        bus.write32(HOST_FLASH_ADDR, 0x20).unwrap();
        bus.write32(HOST_FLASH_PROGRAM, 0x5a).unwrap();
        assert_eq!(bus.read8(XIP_BASE + 0x20).unwrap(), 0x5a);
        bus.write32(HOST_FLASH_ADDR, 0x20).unwrap();
        bus.write32(HOST_FLASH_PROGRAM, 0xf0).unwrap();
        assert_eq!(bus.read8(XIP_BASE + 0x20).unwrap(), 0x50);
        bus.write8(XIP_BASE + 0x20, 0xff).unwrap();
        assert_eq!(bus.read8(XIP_BASE + 0x20).unwrap(), 0x50);
    }

    #[test]
    fn nor_sector_erase_restores_ff() {
        let mut bus = Phy6252Bus::new(vec![0; SRAM_SIZE], vec![0xff; XIP_SIZE]);
        bus.write32(HOST_FLASH_ADDR, 0x1234).unwrap();
        bus.write32(HOST_FLASH_PROGRAM, 0).unwrap();
        assert_eq!(bus.read8(XIP_BASE + 0x1234).unwrap(), 0);
        bus.write32(HOST_FLASH_ADDR, 0x1234).unwrap();
        bus.write32(HOST_FLASH_ERASE, 1).unwrap();
        assert_eq!(bus.read8(XIP_BASE + 0x1234).unwrap(), 0xff);
    }
}
